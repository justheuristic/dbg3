...
class RobertaOutput(nn.Module):
    def __init__(self, config):
        ...
        self.ln_type = config.ln_type
        if config.ln_type == 'post':
            self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        elif config.ln_type == 'pre':
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    config_class = RobertaFixupConfig
    base_model_prefix = "roberta"

    # Copied from transformers.models.bert.modeling_bert.BertPreTrainedModel._init_weights
    def _init_weights(self, module):
        """ Initialize the weights """
        if self.config.use_fixup:
            ...
            if isinstance(module, RobertaSelfOutput):
                nn.init.xavier_uniform_(module.dense.weight)
                module.dense.weight.data = (0.67 * num_layers ** (- 1 / 4)) * module.dense.weight.data
